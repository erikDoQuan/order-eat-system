...
export default function Navbar() {
  ...
  return (
    <nav
      ...
      style={{ boxShadow: '0 2px 12px 0 #00502420' }}
    >
      {/* Dòng trên: logo + tên + user + đăng nhập */}
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
        <div className="flex min-w-[220px] items-center gap-3">
          <img src="/logo.png" alt="Logo" className="h-11 w-11 rounded-full border-2 border-[#005024] bg-white object-contain shadow" />
          <span className="text-2xl font-extrabold tracking-wide" style={{ color: '#005024' }}>
            BẾP CỦA MẸ
          </span>
        </div>
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 pb-2">
        <div className="flex w-full items-center justify-between rounded-[15px] bg-[#005024] px-4 py-2 shadow">
          {/* Menu trung tâm */}
          <ul className="flex max-w-[80%] flex-wrap items-center gap-x-3 text-sm font-semibold text-white">
            {navItems.map(item =>
              item.dropdown ? (
                <li key={item.label} className="group relative">
                  <button type="button" className="flex items-center rounded-lg px-3 py-1.5 transition hover:bg-[#003d1a]">
                    {item.label}
                    ...
                    {item.dropdown.map(drop => (
                      <li key={drop.path}>
                        <NavLink
                          to={drop.path}
                          className={({ isActive }) =>
                            `block rounded-xl px-5 py-3 transition hover:bg-[#e6f4ed] ${isActive ? 'font-bold text-[#005024]' : 'text-[#005024]'}`
                          }
                        >
                          {drop.label}
                        </NavLink>
                ...
                <li key={item.path}>
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `rounded-xl px-3 py-1.5 transition hover:bg-[#003d1a] ${isActive ? 'font-bold text-white' : 'text-white'}`
                    }
                  >
                    {item.label}
                  </NavLink>
          ...
          <NavLink
            to="/cart"
            className="ml-auto flex items-center gap-2 rounded-xl border-2 border-white bg-white px-4 py-2 transition hover:shadow-lg"
          >
            <ShoppingCart size={20} className="text-[#005024]" />
            <span className="text-sm font-bold text-[#00431b]">Giỏ hàng</span>
          </NavLink>
        </div>
      </div>
    </nav>
...
const navItems = [
  {
    ...
    dropdown: [
      ...
      { label: 'Pizza thập cảnh', path: '/dishes/new' },
    ],
  },
